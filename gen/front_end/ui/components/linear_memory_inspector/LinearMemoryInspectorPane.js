...
const str_ = i18n.i18n.registerUIStrings(
  "ui/components/linear_memory_inspector/LinearMemoryInspectorPane.ts",
  UIStrings
);
...
export class LinearMemoryInspectorPaneImpl extends Common.ObjectWrapper.eventMixin(
  UI.Widget.VBox
) {
  #tabbedPane;
  ...
  reveal(tabId, address) {
    ...
    if (!view) {
      throw new Error(
        `No linear memory inspector view for given tab id: ${tabId}`
      );
    }
...
class LinearMemoryInspectorView extends UI.Widget.VBox {
  ...
  refreshData() {
    void LinearMemoryInspectorController.getMemoryForAddress(
      this.#memoryWrapper,
      this.#address
    ).then(({ memory, offset }) => {
      let valueTypes;
      ...
      if (this.firstTimeOpen) {
        const settings =
          LinearMemoryInspectorController.instance().loadSettings();
  ...
  #memoryRequested(event) {
    const { start, end, address, all } = event.data;
    ...
    if (all) {
      ...
      const fun = async () => {
        const memLength = this.#memoryWrapper.length();
        const binaryString = this.#memoryWrapper;

        console.log(binaryString);
        console.log(
          "🚀 ~ file: LinearMemoryInspectorPane.js ~ line 201 ~ LinearMemoryInspectorView ~ fun ~ binaryString",
          binaryString
        );
    ...
    void LinearMemoryInspectorController.getMemoryRange(
      this.#memoryWrapper,
      start,
      end
    ).then((memory) => {
