...
const UIStrings = {
   ...
   */
  addressHasToBeANumberBetweenSAnd:
    "Address has to be a number between {PH1} and {PH2}",
};
const str_ = i18n.i18n.registerUIStrings(
  "ui/components/linear_memory_inspector/LinearMemoryInspector.ts",
  UIStrings
);
...
export class MemoryRequestEvent extends Event {
  ...
  data;
  constructor(start, end, address, all = false) {
    super(MemoryRequestEvent.eventName);
    this.data = { start, end, address, all };
  }
}
...
export class LinearMemoryInspector extends HTMLElement {
  ...
  set data(data) {
    if (
      data.address < data.memoryOffset ||
      data.address > data.memoryOffset + data.memory.length ||
      data.address < 0
    ) {
  ...
  #handleDump() {
    const { start, end } = this.#getPageRangeForAddress(
      this.#address,
      this.#numBytesPerPage
    );

    this.dispatchEvent(new MemoryRequestEvent(start, end, this.#address, true));
  }
  ...
  #render() {
    ...
    const navigatorAddressIsValid = this.#isValidAddress(
      navigatorAddressToShow
    );
    ...
    render(
      ...
      <div class="view">
        <${LinearMemoryNavigator.litTagName}
          .data=${{
            ...
            mode: this.#currentNavigatorMode,
            error: errorMsg,
            canGoBackInHistory,
        ...
        <button @click=${this.#handleDump} > dumpppppp </button>
        <${LinearMemoryViewer.litTagName}
          .data=${{
            memory: this.#memory.slice(
              start - this.#memoryOffset,
      ...
      <div class="value-interpreter">
        <${LinearMemoryValueInterpreter.litTagName}
          .data=${{
            value: this.#memory.slice(
              this.#address - this.#memoryOffset,
              this.#address + VALUE_INTEPRETER_MAX_NUM_BYTES
            ).buffer,
  ...
  #onJumpToPointerAddress(e) {
    ...
    e.stopPropagation();
    this.#currentNavigatorMode = "Submitted" /* Submitted */;
    const addressInRange = Math.max(
      0,
      Math.min(e.data, this.#outerMemoryLength - 1)
    );
  ...
  #isValidAddress(address) {
    ...
    return (
      ...
      newAddress >= 0 &&
      newAddress < this.#outerMemoryLength
    );
  }
  #onAddressChange(e) {
  ...
  #navigateHistory(e) {
    return e.data === "Forward" /* Forward */
      ? this.#history.rollover()
      : this.#history.rollback();
  }
  #navigatePage(e) {
    const newAddress =
      e.data === "Forward" /* Forward */
        ? this.#address + this.#numBytesPerPage
        : this.#address - this.#numBytesPerPage;
    const addressInRange = Math.max(
      0,
      Math.min(newAddress, this.#outerMemoryLength - 1)
    );
    this.#jumpToAddress(addressInRange);
  }
  ...
  #getPageRangeForAddress(address, numBytesPerPage) {
    ...
    const pageEndAddress = Math.min(
      pageStartAddress + numBytesPerPage,
      this.#outerMemoryLength
    );
  ...
  #update() {
    ...
    if (
      start < this.#memoryOffset ||
      end > this.#memoryOffset + this.#memory.length
    ) {
      this.dispatchEvent(new MemoryRequestEvent(start, end, this.#address));
    } else {
      this.#render();
  ...
  #setAddress(address) {
    ...
    const historyEntry = new AddressHistoryEntry(address, () =>
