...
describe('drawGridLineNumbers label placement', () => {
    ...
    for (const { description, config, bounds, expectedLabels } of TESTS) {
        it(description, () => drawGridLineNumbersAndAssertLabels(config, bounds, { canvasWidth: 800, canvasHeight: 600 }, 0, expectedLabels));
    }
});
...
describe('drawGridLineNumbers label skipping logic', () => {
    ...
    it('skips labels on all sides when they are too close to each other', () => {
        drawGridLineNumbersAndAssertLabels({
            positiveRowLineNumberPositions: [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200].map(y => ({ x: 100, y: y + 100 })),
            negativeRowLineNumberPositions: [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200].map(y => ({ x: 300, y: y + 100 })),
            positiveColumnLineNumberPositions: [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200].map(x => ({ y: 100, x: x + 100 })),
            negativeColumnLineNumberPositions: [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200].map(x => ({ y: 300, x: x + 100 })),
        }, {
            minX: 100,
            maxX: 300,
            minY: 100,
...
describe('normalizePositionData', () => {
    ...
    it('prefers line names over line numbers when present', () => {
        ...
        assert.strictEqual(data.rows.positive.positions.length, 1, 'There should be only one row offset since there is only one name');
        assert.strictEqual(data.columns.positive.positions.length, 2, 'There should be 2 column offsets since there are 2 names');
    });
...
describe('drawGridAreaNames  label placement with vertical writing mode', () => {
    beforeEach(initFrameForGridLabels);
    const areaBounds = [{ name: 'foo', bounds: { allPoints: [{ x: 20, y: 30 }, { x: 100, y: 30 }, { x: 100, y: 50 }, { x: 20, y: 50 }] } }];
    const TESTS = [
        {
...
describe('drawMultipleGridLabels', () => {
    it('can set labels on multiple grid nodes', () => {
        ...
        drawMultipleGridLineNumbersAndAssertLabels(configs.map(config => ({ config, layerId: config.layerId })), bounds, { canvasWidth: 800, canvasHeight: 600 }, expectedLayers);
    });
});
...
describe('drawGridLineNames', () => {
    ...
    for (const { description, rowLineNameOffsets, columnLineNameOffsets, deviceEmulationFactor, expectedLabels } of TESTS) {
        it(description, () => drawGridLineNamesAndAssertLabels({
            gridHighlightConfig: {
                showLineNames: true,
            },
            rowLineNameOffsets,
...
//# sourceMappingURL=css_grid_label_helpers_test.js.map