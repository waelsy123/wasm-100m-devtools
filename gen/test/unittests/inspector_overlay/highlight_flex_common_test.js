...
function createPathCommands(...points) {
    if (points.length !== 8) {
...
describe('getLinesAndItemsQuads', () => {
    it('creates the right number of line and item quads', () => {
        const lineQuads = getLinesAndItemsQuads(createPathCommands(0, 0, 100, 0, 100, 100, 0, 100), [
            [
    ...
    it('creates a line quad as big as the container when there is only one line', () => {
        const lineQuads = getLinesAndItemsQuads(createPathCommands(0, 0, 10, 0, 10, 10, 0, 10), [[createItem(2, 2, 8, 2, 8, 8, 2, 8)]], true, false);
    ...
    it('creates quads for flex lines that extend to the edges of the container in the main direction', () => {
        const lineQuadsRowDirection = getLinesAndItemsQuads(createPathCommands(0, 0, 100, 0, 100, 80, 0, 80), [
            [
                createItem(10, 10, 30, 10, 30, 30, 10, 30),
                createItem(40, 10, 60, 10, 60, 30, 40, 30),
                createItem(70, 10, 90, 10, 90, 30, 70, 30),
        ...
        const lineQuadsColumnDirection = getLinesAndItemsQuads(createPathCommands(0, 0, 50, 0, 50, 70, 0, 70), [
            [
    ...
    it('creates normal and extended quads for items', () => {
        const lineQuads = getLinesAndItemsQuads(createPathCommands(0, 0, 70, 0, 70, 40, 0, 40), [
            [
                createItem(10, 10, 30, 10, 30, 30, 10, 30),
                createItem(40, 10, 60, 10, 60, 30, 40, 30),
            ],
        ], true, false);
        assert.deepStrictEqual(lineQuads[0].items[0], createQuad(10, 10, 30, 10, 30, 30, 10, 30), 'The first flex item quad matches the item');
        assert.deepStrictEqual(lineQuads[0].items[1], createQuad(40, 10, 60, 10, 60, 30, 40, 30), 'The second flex item quad matches the item');
        assert.deepStrictEqual(lineQuads[0].extendedItems[0], createQuad(10, 0, 30, 0, 30, 40, 10, 40), 'The first flex item extended quad extends to the cross edge of the flex line');
        assert.deepStrictEqual(lineQuads[0].extendedItems[1], createQuad(40, 0, 60, 0, 60, 40, 40, 40), 'The second flex item extended quad extends to the cross edge of the flex line');
    });
    it('creates correct quads with transformed layout', () => {
        const lineQuads = getLinesAndItemsQuads(createPathCommands(10, 70, 70, 10, 110, 50, 50, 110), [
            [
...
describe('getGapQuadBetweenQuads', () => {
    it('creates a quad between 2 quads stacked either vertically or horizontally, also when reversed', () => {
        const quadV = getGapQuadBetweenQuads(createQuad(0, 0, 60, 0, 60, 10, 0, 10), createQuad(0, 20, 60, 20, 60, 30, 0, 30), 10, true, false);
        assert.deepStrictEqual(quadV, createQuad(0, 10, 60, 10, 60, 20, 0, 20));
        const quadVReversed = getGapQuadBetweenQuads(createQuad(0, 20, 60, 20, 60, 30, 0, 30), createQuad(0, 0, 60, 0, 60, 10, 0, 10), 10, true, true);
        assert.deepStrictEqual(quadVReversed, quadV);
        const quadH = getGapQuadBetweenQuads(createQuad(0, 0, 10, 0, 10, 50, 50, 0), createQuad(20, 0, 30, 0, 30, 50, 50, 20), 10, false, false);
        assert.deepStrictEqual(quadH, createQuad(10, 0, 20, 0, 20, 50, 10, 50));
        const quadHReversed = getGapQuadBetweenQuads(createQuad(20, 0, 30, 0, 30, 50, 50, 20), createQuad(0, 0, 10, 0, 10, 50, 50, 0), 10, false, true);
        assert.deepStrictEqual(quadHReversed, quadH);
    });
    it('works when the gap is smaller than the distance between the quads', () => {
        const quad = getGapQuadBetweenQuads(createQuad(0, 0, 30, 0, 30, 20, 0, 20), createQuad(0, 50, 30, 50, 30, 70, 0, 70), 10, true, false);
        assert.deepStrictEqual(quad, createQuad(0, 30, 30, 30, 30, 40, 0, 40));
    });
    it('works when the quads are transformed', () => {
        const quad = getGapQuadBetweenQuads(createQuad(0, 20, 20, 0, 40, 20, 20, 40), createQuad(50, 70, 70, 50, 80, 60, 60, 80), 10, true, false);
...
describe('uniteQuads', () => {
    it('creates a quad that is big enough to contain the 2 passed quads', () => {
        const quad = uniteQuads(createQuad(0, 20, 10, 20, 10, 30, 0, 30), createQuad(20, 10, 40, 10, 40, 40, 20, 40), true, false);
    ...
    it('can be called multiple times with the previously united quad to construct a flex line out of flex items', () => {
        let quad = uniteQuads(createQuad(0, 20, 10, 20, 10, 30, 0, 30), createQuad(20, 10, 40, 10, 40, 40, 20, 40), true, false);
        quad = uniteQuads(quad, createQuad(60, 30, 90, 30, 90, 50, 60, 50), true, false);
        quad = uniteQuads(quad, createQuad(130, 0, 180, 0, 180, 30, 130, 30), true, false);
        assert.deepStrictEqual(quad, createQuad(0, 0, 180, 0, 180, 50, 0, 50));
    });
    it('also works when the quads are transformed', () => {
        const quad = uniteQuads(createQuad(0, 20, 20, 0, 40, 20, 20, 40), createQuad(50, 70, 70, 50, 80, 60, 60, 80), false, false);
        assert.deepStrictEqual(quad, createQuad(0, 20, 20, 0, 80, 60, 60, 80));
    });
});
describe('growQuadToEdgesOf', () => {
    it('works horizontally', () => {
        const quad = growQuadToEdgesOf(createQuad(10, 10, 20, 10, 20, 20, 10, 20), createQuad(0, 0, 60, 0, 60, 30, 0, 30), true);
        assert.deepStrictEqual(quad, createQuad(0, 10, 60, 10, 60, 20, 0, 20));
    });
    it('works vertically', () => {
        const quad = growQuadToEdgesOf(createQuad(10, 10, 20, 10, 20, 20, 10, 20), createQuad(0, 0, 60, 0, 60, 30, 0, 30), false);
        assert.deepStrictEqual(quad, createQuad(10, 0, 20, 0, 20, 30, 10, 30));
    });
    it('works with transformed quads', () => {
        const quad = growQuadToEdgesOf(createQuad(30, 60, 60, 30, 80, 50, 50, 80), createQuad(10, 60, 60, 10, 100, 50, 50, 100), true);
        assert.deepStrictEqual(quad, createQuad(20, 70, 70, 20, 90, 40, 40, 90));
    });
});
describe('getColinearPointAtDistance', () => {
    function assertPoint(p1, p2, distance, expected) {
...
describe('distance', () => {
    function assertDistance(p1, p2, expected) {
...
describe('intersectSegments', () => {
    function assertIntersection(s1, s2, expected) {
...
//# sourceMappingURL=highlight_flex_common_test.js.map