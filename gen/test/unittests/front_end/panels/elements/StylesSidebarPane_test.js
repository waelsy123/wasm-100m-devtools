...
describeWithRealConnection('StylesSidebarPane', async () => {
    ...
    it('unescapes CSS strings', () => {
        ...
        assert.strictEqual(Elements.StylesSidebarPane.unescapeCssString(String.raw `"\0_\DBFF_\DFFF_\110000"`), '"\uFFFD_\uFFFD_\uFFFD_\uFFFD"', 'U+0000, lone surrogates, and values above U+10FFFF should become U+FFFD');
        assert.strictEqual(Elements.StylesSidebarPane.unescapeCssString(String.raw `"_\D83C\DF08_"`), '"_\uFFFD\uFFFD_"', 'surrogates should not be combined');
    ...
    it('tracks property changes with formatting', async () => {
        ...
        cssModel.styleSheetHeaderForId = () => ({
            lineNumberInSource: (line) => line,
            columnNumberInSource: (_line, column) => column,
            cssModel: () => cssModel,
            resourceURL,
        ...
        cssWorksapceBinding.addSourceMapping({
            rawLocationToUILocation: (loc) => new Workspace.UISourceCode.UILocation(uiSourceCode, loc.lineNumber, loc.columnNumber),
            uiLocationToRawLocations: (_) => [],
        });
        const cssProperty = {
            ownerStyle: {
...
//# sourceMappingURL=StylesSidebarPane_test.js.map