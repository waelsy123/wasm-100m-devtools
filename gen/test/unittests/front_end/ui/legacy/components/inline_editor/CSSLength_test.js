...
describe('CSSLength', () => {
    ...
    it('can +/- length values when the value is dragged', () => {
        ...
        assert.strictEqual(lengthText, '33px', 'length value should decrease by 10 when the mouse is dragged to left with shift key');
        const mousemoveRightAlt = new MouseEvent('mousemove', {
            clientX: ++mousePositionX,
            altKey: true,
        });
        document.dispatchEvent(mousemoveRightAlt);
        assert.strictEqual(lengthText, '33.1px', 'length value should decrease by 10 when the mouse is dragged to left with shift key');
    });
    describe('#CSSLengthUtils', () => {
        it('parses CSS properties with length correctly', () => {
            assert.deepEqual(InlineEditor.CSSLengthUtils.parseText('42px'), { value: 42, unit: "px" /* PIXEL */ });
            assert.deepEqual(InlineEditor.CSSLengthUtils.parseText('-5vw'), { value: -5, unit: "vw" /* VW */ });
...
//# sourceMappingURL=CSSLength_test.js.map