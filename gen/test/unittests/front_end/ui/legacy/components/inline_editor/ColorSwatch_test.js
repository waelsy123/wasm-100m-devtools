...
function createSwatch(color, formatOrUseUserSetting, tooltip) {
...
describeWithLocale('ColorSwatch', () => {
    ...
    it('accepts colors as color objects', () => {
        const swatch = createSwatch(Common.Color.Color.parse('red'));
        assertSwatch(swatch, {
            backgroundColor: 'red',
            colorTextInSlot: 'red',
        });
    ...
    it('renders text only for invalid colors provided as text', () => {
        ...
        assertShadowRoot(swatch.shadowRoot);
        assert.strictEqual(swatch.shadowRoot.querySelectorAll('.color-swatch').length, 0, 'There is no swatch in the component');
    ...
    it('dispatches an event when the format changes', () => {
        ...
        swatch.addEventListener(InlineEditor.ColorSwatch.FormatChangedEvent.eventName, (e) => {
...
//# sourceMappingURL=ColorSwatch_test.js.map