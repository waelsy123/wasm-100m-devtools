...
function testHighlight(code, mimeType) {
    return async () => {
        ...
        highlighter.highlight((text, style) => {
            assert.strictEqual(JSON.stringify([text, style.replace(/\btoken-/g, '').split(' ').sort().join('&')]), JSON.stringify(tokens[i++] || ['', '']));
        });
    };
}
...
//# sourceMappingURL=CodeHighlighter_test.js.map