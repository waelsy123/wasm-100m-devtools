...
async function renderTreeOutline({ tree, defaultRenderer, filter, }) {
    ...
    const data = {
        ...
        defaultRenderer: defaultRenderer ||
            ((node) => LitHtml.html `${node.treeNodeData}`),
        filter,
    };
    component.data = data;
...
describe('TreeOutline', () => {
    ...
    it('caches async child nodes and only fetches them once', async () => {
        const fetchChildrenSpy = sinon.spy(async () => {
            return [
                {
                    treeNodeData: 'EMEA',
                    id: '1',
    ...
    describe('navigating with keyboard', () => {
        ...
        describe('pressing the END key', () => {
            it('takes the user to the last visible node if they are all expanded', async () => {
                ...
                await coordinator.done();
                assert.strictEqual(getFocusableTreeNode(shadowRoot), 
        ...
        describe('pressing the UP arrow', () => {
            ...
            it('moves focus to the parent\'s last child if there are no previous siblings and the parent is expanded', async () => {
                const { component, shadowRoot } = await renderTreeOutline({
                    tree: basicTreeData,
                });
                await component.expandRecursively();
            ...
            it('moves focus to the parent\'s deeply nested last child if there are no previous siblings and the parent has children that are expanded', async () => {
                const { component, shadowRoot } = await renderTreeOutline({
                    tree: basicTreeData,
                });
                await component.expandRecursively(Number.POSITIVE_INFINITY);
...
//# sourceMappingURL=TreeOutline_test.js.map