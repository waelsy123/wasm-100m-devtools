...
describeWithEnvironment('TextEditor', () => {
    ...
    describe('autocompletion', () => {
        it('can complete builtins and keywords', async () => {
            ...
            const result = await TextEditor.JavaScript.javascriptCompletionSource(new CodeMirror.CompletionContext(state, 1, false));
        ...
        it('recognizes expression queries', async () => {
            await testQueryType('foo', 3, 0 /* Expression */, 'foo');
            await testQueryType('foo ', 4, 0 /* Expression */, '');
            await testQueryType('let', 3, 0 /* Expression */, 'let');
        });
        it('recognizes propery name queries', async () => {
            await testQueryType('foo.bar', 7, 1 /* PropertyName */, 'bar', 'foo.bar');
            await testQueryType('foo.', 4, 1 /* PropertyName */, '', 'foo.');
            await testQueryType('if (foo.', 8, 1 /* PropertyName */, '', 'foo.');
            await testQueryType('foo.', 4, 1 /* PropertyName */, '', 'foo.');
            await testQueryType('foo.\n', 5, 1 /* PropertyName */, '', 'foo.');
            await testQueryType('new foo.bar().', 14, 1 /* PropertyName */, '', 'new foo.bar().');
            await testQueryType('foo?.', 5, 1 /* PropertyName */, '', 'foo?.');
            await testQueryType('foo?.b', 6, 1 /* PropertyName */, 'b', 'foo?.b');
        });
        it('recognizes property expression queries', async () => {
            await testQueryType('foo[', 4, 2 /* PropertyExpression */, '', 'foo[');
            await testQueryType('foo[ ', 5, 2 /* PropertyExpression */, '', 'foo[');
            await testQueryType('foo["ba', 7, 2 /* PropertyExpression */, '"ba', 'foo["ba');
        });
        describe('potential map key retrievals', () => {
            it('recognizes potential maps', async () => {
                await testQueryType('foo.get(', 8, 3 /* PotentiallyRetrievingFromMap */, '', 'foo');
                await testQueryType('foo\n.get(', 9, 3 /* PotentiallyRetrievingFromMap */, '', 'foo');
            });
            it('leaves other expressions as-is', async () => {
                await testQueryType('foo.method(', 11, 0 /* Expression */);
                await testQueryType('5 + (', 5, 0 /* Expression */);
                await testQueryType('functionCall(', 13, 0 /* Expression */);
            });
        });
...
//# sourceMappingURL=TextEditor_test.js.map