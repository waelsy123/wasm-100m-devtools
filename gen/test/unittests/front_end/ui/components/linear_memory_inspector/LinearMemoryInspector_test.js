...
describeWithLocale('LinearMemoryInspector', () => {
    function getViewer(component) {
        return getElementWithinComponent(component, VIEWER_SELECTOR, LinearMemoryInspectorModule.LinearMemoryViewer.LinearMemoryViewer);
    }
    function getNavigator(component) {
        return getElementWithinComponent(component, NAVIGATOR_SELECTOR, LinearMemoryInspectorModule.LinearMemoryNavigator.LinearMemoryNavigator);
    }
    function getValueInterpreter(component) {
        return getElementWithinComponent(component, INTERPRETER_SELECTOR, LinearMemoryInspectorModule.LinearMemoryValueInterpreter.LinearMemoryValueInterpreter);
    }
    function setUpComponent() {
        ...
        const data = {
            ...
            valueTypes: new Set(LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.getDefaultValueTypeMapping().keys()),
        };
        component.data = data;
    ...
    it('only saves history entries if addresses differ', async () => {
        ...
        for (const index of byteIndices) {
            const byteSelectedPromise = getEventPromise(viewer, 'byteselected');
    ...
    it('can navigate addresses back and forth in history', async () => {
        ...
        for (let i = historyLength - 1; i >= 0; --i) {
            const currentByteValue = getElementWithinComponent(viewer, VIEWER_BYTE_CELL_SELECTOR + '.selected', HTMLSpanElement);
    ...
    it('can change endianness settings on event', () => {
        ...
        assert.deepEqual(select.value, "Little Endian" /* Little */);
        const endianSetting = "Big Endian" /* Big */;
    ...
    it('updates current address if user triggers a jumptopointeraddress event', () => {
        const { component, data } = setUpComponent();
        data.valueTypes = new Set(["Pointer 32-bit" /* Pointer32 */]);
        data.memory = new Uint8Array([2, 0, 0, 0]);
        data.outerMemoryLength = data.memory.length;
        data.address = 0;
        data.endianness = "Little Endian" /* Little */;
        component.data = data;
        const interpreter = getValueInterpreter(component);
        const display = getElementWithinComponent(interpreter, 'devtools-linear-memory-inspector-interpreter-display', LinearMemoryInspectorModule.ValueInterpreterDisplay.ValueInterpreterDisplay);
    ...
    it('triggers MemoryRequestEvent on refresh', async () => {
        ...
        const eventPromise = getEventPromise(component, 'memoryrequest');
    ...
    it('triggers event on address change when byte is selected', async () => {
        const { component, data } = setUpComponent();
        const eventPromise = getEventPromise(component, 'addresschanged');
        const viewer = getViewer(component);
        const bytes = getElementsWithinComponent(viewer, VIEWER_BYTE_CELL_SELECTOR, HTMLSpanElement);
        const numBytesPerPage = bytes.length;
    ...
    it('triggers event on settings changed when value type is changed', async () => {
        ...
        const valueType = "Integer 16-bit" /* Int16 */;
        interpreter.dispatchEvent(new LinearMemoryInspectorModule.LinearMemoryValueInterpreter.ValueTypeToggledEvent(valueType, false));
        const event = await eventPromise;
    ...
    it('triggers event on settings changed when value type mode is changed', async () => {
        ...
        const eventPromise = getEventPromise(component, 'settingschanged');
        const valueType = "Integer 16-bit" /* Int16 */;
        const valueTypeMode = "hex" /* Hexadecimal */;
        interpreter.dispatchEvent(new LinearMemoryInspectorModule.ValueInterpreterDisplay.ValueTypeModeChangedEvent(valueType, valueTypeMode));
        const event = await eventPromise;
    ...
    it('triggers event on settings changed when endianness is changed', async () => {
        ...
        const interpreter = getValueInterpreter(component);
        const eventPromise = getEventPromise(component, 'settingschanged');
        const endianness = "Big Endian" /* Big */;
        interpreter.dispatchEvent(new LinearMemoryInspectorModule.LinearMemoryValueInterpreter.EndiannessChangedEvent(endianness));
        const event = await eventPromise;
        assert.strictEqual(event.data.endianness, endianness);
    });
    it('formats a hexadecimal number', () => {
        const number = 23;
        assert.strictEqual(LinearMemoryInspectorModule.LinearMemoryInspectorUtils.toHexString({ number, pad: 0, prefix: false }), '17');
    });
    it('formats a hexadecimal number and adds padding', () => {
        const number = 23;
        assert.strictEqual(LinearMemoryInspectorModule.LinearMemoryInspectorUtils.toHexString({ number, pad: 5, prefix: false }), '00017');
    });
    it('formats a hexadecimal number and adds prefix', () => {
        const number = 23;
        assert.strictEqual(LinearMemoryInspectorModule.LinearMemoryInspectorUtils.toHexString({ number, pad: 5, prefix: true }), '0x00017');
    });
...
//# sourceMappingURL=LinearMemoryInspector_test.js.map