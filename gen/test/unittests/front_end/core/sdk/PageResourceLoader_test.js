...
describeWithLocale('PageResourceLoader', () => {
    ...
    it('loads resources correctly', async () => {
        const loader = SDK.PageResourceLoader.PageResourceLoader.instance({ forceNew: true, loadOverride: load, maxConcurrentLoads: 500, loadTimeout: 30000 });
        const loading = [
    ...
    it('deals with page reloads correctly', async () => {
        const loader = SDK.PageResourceLoader.PageResourceLoader.instance({ forceNew: true, loadOverride: load, maxConcurrentLoads: 1, loadTimeout: 30000 });
        const loading = [
    ...
    it('handles the load timeout correctly', async () => {
        ...
        const loader = SDK.PageResourceLoader.PageResourceLoader.instance({ forceNew: true, loadOverride: load, maxConcurrentLoads: 2, loadTimeout: 30 });
        const loading = [
            loader.loadResource(foo1Url, initiator).catch(e => e.message),
            loader.loadResource(foo2Url, initiator).catch(e => e.message),
            loader.loadResource(foo3Url, initiator).catch(e => e.message),
        ...
        assert.isTrue(results.every(x => x === 'Load canceled due to load timeout'), 'All loads should have a exceeded the load timeout');
        assert.deepEqual(loader.getNumberOfResources(), { loading: 0, queued: 0, resources: 3 });
    });
...
//# sourceMappingURL=PageResourceLoader_test.js.map