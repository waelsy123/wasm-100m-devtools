...
describe('RemoteObject', () => {
    describe('fromLocalObject', () => {
        ...
        it('can handle a nested object', () => {
            const remoteObject = SDK.RemoteObject.RemoteObject.fromLocalObject({ foo: 'bar', baz: { nested: 34 }, another: { nested: { object: true } } });
            assert.deepEqual(remoteObject.type, 'object');
            assert.deepEqual(remoteObject.subtype, undefined);
...
describe('RemoteObjectProperty', () => {
    describe('isAccessorProperty', () => {
        it('correctly handles data properties', () => {
            const property = new SDK.RemoteObject.RemoteObjectProperty('x', SDK.RemoteObject.RemoteObject.fromLocalObject('some string'));
    ...
    describe('match', () => {
        ...
        it('correctly handles properties with a RemoteObject null value', () => {
            const property = new SDK.RemoteObject.RemoteObjectProperty('bar', SDK.RemoteObject.RemoteObject.fromLocalObject(null));
            assert.isFalse(property.match({ includeNullOrUndefinedValues: false, regex: null }));
            assert.isTrue(property.match({ includeNullOrUndefinedValues: true, regex: null }));
        ...
        it('correctly handles properties with a RemoteObject undefined value', () => {
            const property = new SDK.RemoteObject.RemoteObjectProperty('x', SDK.RemoteObject.RemoteObject.fromLocalObject(undefined));
            assert.isFalse(property.match({ includeNullOrUndefinedValues: false, regex: null }));
            assert.isTrue(property.match({ includeNullOrUndefinedValues: true, regex: null }));
        ...
        it('correctly handles properties with a RemoteObject object value', () => {
            const property = new SDK.RemoteObject.RemoteObjectProperty('obj', SDK.RemoteObject.RemoteObject.fromLocalObject({ x: 1 }));
        ...
        it('correctly matches property values\' descriptions', () => {
            const property = new SDK.RemoteObject.RemoteObjectProperty('foo', SDK.RemoteObject.RemoteObject.fromLocalObject('this is a bar'));
...
//# sourceMappingURL=RemoteObject_test.js.map