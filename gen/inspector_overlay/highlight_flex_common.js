...
export function drawLayoutFlexItemHighlight(highlight, itemPath, context, deviceScaleFactor, canvasWidth, canvasHeight, emulationScaleFactor) {
...
function drawFlexibilityArrow(highlight, itemQuad, baseSizeQuad, context, emulationScaleFactor) {
    ...
    drawPathWithLineStyle(context, buildPath(path, emptyBounds(), emulationScaleFactor), config.flexibilityArrow, FLEXIBILITY_ARROW_THICKNESS);
...
export function drawLayoutFlexContainerHighlight(highlight, context, deviceScaleFactor, canvasWidth, canvasHeight, emulationScaleFactor) {
    ...
    drawFlexAlignment(highlight, context, emulationScaleFactor, lineQuads, lines.map(line => line.map(item => item.baseline)));
}
...
function drawFlexAlignmentForLine(highlight, context, emulationScaleFactor, lineQuad, itemQuads, itemBaselines) {
    ...
    for (const points of linesToDraw) {
        ...
        drawPathWithLineStyle(context, buildPath(path, emptyBounds(), emulationScaleFactor), crossAlignment, ALIGNMENT_LINE_THICKNESS);
...
export function getColinearPointAtDistance(p1, p2, distance) {
